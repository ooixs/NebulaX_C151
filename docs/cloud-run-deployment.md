...
`--min 0` permits scale-to-zero. `--max 1` is deliberate: the current app stores recent runs and
download data in one process's memory. Restricting normal operation to one instance avoids most
...
The local app accepts batches up to 120 MB. This container uses a Python HTTP/1 server, while
Cloud Run limits HTTP/1 requests to 32 MiB. Multipart encoding adds overhead, so keep each cloud
upload comfortably below that limit (for example, below 30 MiB) and split larger file sets into
separate batches. See the current [Cloud Run quotas and limits](https://docs.cloud.google.com/run/quotas)
before deployment.
